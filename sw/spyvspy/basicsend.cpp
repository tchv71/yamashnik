...
{
	char command[40];
	sprintf_s(command, " _nete:DefUsr=&H%.4x:?Usr(0):_neti", defusr);
...
int BasicSender::sendROMSection(FILE* file, int sectionSize, int patchAddr, uint8_t patchVal)
{
	...
	uint16_t end = start + sectionSize + (uint16_t)sizeof(ROM2BIN_startCode) - 1;
...
int BasicSender::netSend(int nfiles, char* file[])
{
	for (int fileIdx = 0; fileIdx < nfiles; fileIdx++) {
		...
		FILE* infile = nullptr;
		fopen_s(&infile, file[fileIdx], "rb");
		if (infile == 0) {
