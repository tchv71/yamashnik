...
#ifndef __APPLE__
#include <limits.h>
#define GLOB_MAXPATH MAX_PATH//PATH_MAX
#endif
...
void Spy::extractExePath() {
	...
	size_t pathlen = pathend - fullPath;
	m_ExePath = _strdup(fullPath);
}

int Spy::loadFile(const char* filename, uint8_t** buf, int expectedSize)
{
	char path[MAX_PATH];// = new char[strlen(m_ExePath) + strlen(filename) + 2];

	if (filename[0] == '/' || filename[0] == '.') {
		sprintf_s(path, "%s", filename);
	}
	else {
		sprintf_s(path, "%s/%s", m_ExePath, filename);
	}
	...
	*buf = new uint8_t[expectedSize];
	...
	int size = (int)fread(*buf, 1, expectedSize, file);
	fclose(file);
...
int Spy::Bootstrap()
{
	...
	{
		char path[MAX_PATH];
		sprintf_s(path, "%s/%s", m_ExePath, STRAPON);
