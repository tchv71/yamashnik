...
private:
	...
	void randomBlockRead() {
		...
		do {
			...
			size_t recordsread = fread(m_res->GetDMA(), recordsize, nrecords, f);
			m_res->SetDMASize((int)recordsread * recordsize);
			m_res->SetAuxData(0, (int)recordsread); // records read
			m_res->SetAuxData(1, recordsread == nrecords ? 0 : 1); // error set if reached EOF
			m_res->GetFCB()->RandomRecord += (int)recordsread;
		} while (0);
		verbose("\n");
	...
	void randomRead() {
		...
		fopen_s(&f, filename, "rb");
		do {
			if ((fileoffset >= (int)m_req->GetFCB()->FileSize) ||
			...
			size_t bytesread = fread(m_res->GetDMA(), 1, recordsize, f);
			...
			m_res->GetFCB()->CurrentRecord = uint8_t(fileoffset % extentsize);
			m_res->GetFCB()->RecordSizeLo = 0;
			m_res->GetFCB()->RecordSizeHi = recordsize;

			m_res->GetFCB()->SetExtent(uint16_t(fileoffset / extentsize));
		} while (0);
		fclose(f);
	...
	void randomWrite(int sequential) {
		...
		if (fileoffset + recordsize > (off_t)m_res->GetFCB()->FileSize)
